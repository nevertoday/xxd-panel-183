...
# XXD Panel 183｜تكوين عضوي أخضر في الزاوية
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-183`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-183 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-183-prompt.en.md) أو [بالصينية](references/xxd-panel-183-prompt.zh-CN.md).
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 183 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
